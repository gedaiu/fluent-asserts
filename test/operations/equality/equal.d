module test.operations.equality.equal;
...
static foreach (Type; StringTypes) {
    ...
    @(Type.stringof ~ " throws exception when strings should not be equal")
    unittest {
        auto msg = ({
...
static foreach (Type; NumericTypes) {
    ...
    @(Type.stringof ~ " throws exception when values should not be equal")
    unittest {
        Type testValue = cast(Type) 40;
        auto msg = ({
...
@("objects without custom opEquals throws exception when should not be equal")
unittest {
    Object testValue = new Object();
    string niceTestValue = SerializerRegistry.instance.niceValue(testValue);
...
unittest {
    Thing testValue = new Thing(1);
    Thing sameTestValue = new Thing(1);
...
unittest {
    Thing testValue = new Thing(1);
    Thing otherTestValue = new Thing(2);
    expect(testValue).to.not.equal(otherTestValue);
}
...
@("objects with custom opEquals throws exception when should not be equal")
unittest {
    Thing testValue = new Thing(1);
    string niceTestValue = SerializerRegistry.instance.niceValue(testValue);
...
@("assoc arrays throws exception when should not be equal")
unittest {
    string[string] testValue = ["b": "2", "a": "1", "c": "3"];
    string niceTestValue = SerializerRegistry.instance.niceValue(testValue);
