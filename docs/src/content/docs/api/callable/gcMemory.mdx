...
Asserts that a callable allocates memory managed by the garbage collector during its execution. Uses `GC.profileStats()` for accurate per-thread allocation tracking.
